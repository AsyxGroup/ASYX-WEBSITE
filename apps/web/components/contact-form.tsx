...
export function ContactForm() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const text = `New enquiry from ${form.name}%0A%0AName: ${form.name}%0AOrganisation: ${form.organisation || "N/A"}%0AEmail: ${form.email}%0APhone: ${form.phone || "N/A"}%0ANeed: ${form.need || "N/A"}%0A%0AMessage:%0A${form.message}`
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-border bg-card p-8 text-center sm:p-12">
        ...
        <button
          onClick={() => {
            setSubmitted(false)
            setForm({ name: "", organisation: "", email: "", phone: "", need: "", message: "" })
          }}
    ...
    <div className="flex flex-col gap-6 rounded-2xl border border-border bg-card p-5 sm:p-6 lg:p-8">
      <div>
        ...
        <div className="mt-2 flex items-center gap-2 text-sm text-primary">
          ...
          Choose WhatsApp or email to contact our team
        </div>
      </div>
      ...
      <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="contact-organisation" className="text-sm font-medium">Organisation</Label>
            <Input
        ...
        <div className="flex flex-col gap-2">
          ...
          <Input
            id="contact-need"
            placeholder="Cybersecurity, software, infrastructure, smart systems..."
            value={form.need}
